//Récupération de la chaine de requête dans l'URL, cela vas nous servir à afficher la campagne selectionné sur la page index
const queryString_url_id = window.location.search
const urlSearchParams = new URLSearchParams(queryString_url_id)
const _id = urlSearchParams.get('_id')

const promiseCampaign = fetch(`http://localhost:3000/api/campaigns/${_id}`)
...
//   .then((res) => {
//     data = res
//     //Récupération, au sein de l'API, de la campagne séléctionné par l'utilisateur 
//     const selectedCampaign_id = data.find((element) => element._id === _id)
...
//          for(j = 0 ; j < selectedCampaign_id.colors.length ; j ++)
//       event.preventDefault()
//       //Mettre le choix de l'utilisateur dans une variable
//       const choixForm = idForm.value
//       //Mettre la quantité dans une variable
//       let choixQuantity = quantity.value
//       //Récupération des valeurs au sein de la variables "optionsCampagne"
//       optionsCampagne = {
...
//       function addToArray() {
//         fromJS.push(optionsCampagne)
//         localStorage.setItem('idJSON', JSON.stringify(fromJS))
//         window.alert("La campagne vient d'être ajouté à votre panier")
//       }
      
//       //Fonction servant à retourner les Campagnes ayant le même ID, afin que le tri soit fait lors de l'ajout au panier
//       function sameId(campaign) {
//         return campaign.selectedCampaign_id === optionsCampagne.selectedCampaign_id
//       }
      
//       ///LA GESTION DU PANIER///
//       if (fromJSON) {
//         fromJS = JSON.parse(fromJSON) //On ajoute dans le tableau JS les Campagnes déjà présents dans le storage
//         for (i = 0; i < fromJS.length; i++){
//           if (optionsCampagne.quantite === '0') {
//             alert('Le quantité séléctionnée est de 0')
//           } else if (
//             optionsCampagne.selectedCampaign_id !== fromJS[i].selectedCampaign_id
//           ) {
//             const quantityToNumber = parseInt(optionsCampagne.quantite)
//             optionsCampagne.quantite = quantityToNumber
...
//               addToArray() //Nous ajoutons donc la Campagne au panier
//               break //Puis nous sortons de la boucle for
//             }
//           } else if (
//             optionsProduit.selectedCampaign_id === fromJS[i].selectedCampaign_id
//           ) {
...
//                 (produit) =>
//                   produit.selectedCampaign_id ===
//                   optionsProduit.selectedCampaign_id,
...
const priceFetching = fetch('http://localhost:3000/api/campaigns')
  .then((res) => res.json())
  .then((res) => {
    data = res
    let fetchedCampaign = data.find((element) => element._id === _id)
    document.getElementById('price').innerHTML = fetchedCampaign.price 
  })
